-- Ensure loan dashboard views return one row per property.
--
-- Some portfolio_properties rows can match the same property via either id or
-- core_property_id. A plain LEFT JOIN multiplies the loan dashboard rows. Use a
-- lateral single-row match instead.
...
WITH ledger AS (
  ...
  LEFT JOIN LATERAL (
    SELECT pp_match.id, pp_match.name
    FROM public.portfolio_properties pp_match
    WHERE pp_match.id = summary.property_id
       OR pp_match.core_property_id = summary.property_id
    ORDER BY
      CASE WHEN pp_match.id = summary.property_id THEN 0 ELSE 1 END,
      pp_match.name,
      pp_match.id
    LIMIT 1
  ) pp ON true
...
LEFT JOIN LATERAL (
  SELECT pp_match.id, pp_match.name
  FROM public.portfolio_properties pp_match
  WHERE pp_match.id = d.property_id
     OR pp_match.core_property_id = d.property_id
  ORDER BY
    CASE WHEN pp_match.id = d.property_id THEN 0 ELSE 1 END,
    pp_match.name,
    pp_match.id
  LIMIT 1
) pp ON true;
