...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    async function loadFinanceSummary() {
      const { data, error } = await supabase
        ...
        .select("portfolio_property_id,name,jahr,einnahmen,ausgaben,mieteingaenge")
        .eq("jahr", year);
  ...
  function getFinanceForRow(row: PortfolioLoanRow): PortfolioFinanceSummary | undefined {
    ...
    return financeSummary.find((item) => {
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      (acc, row) => {
        ...
        const finance = getFinanceForRow(row);
    ...
    <div className="portfolio-page portfolio-page-v11">
      ...
      <section className="portfolio-cashflow-panel">
        <div>
          ...
          <p>Einnahmen und Ausgaben kommen automatisch aus der SQL-View. Die Rendite nutzt den eingetragenen Objektwert; wenn kein Wert gepflegt ist, wird ersatzweise die Restschuld verwendet.</p>
        </div>
                ...
                <div className="portfolio-info-grid refined-info">
                  <section className="portfolio-edit-box">
                    ...
                    <div className="portfolio-input-grid three">
                      <label>Wohnfläche<input value={extra.livingArea} onChange={(e) => updateExtra(row.property_id, "livingArea", e.target.value)} placeholder="z.B. 150 m²" /></label>
                      <label>Zimmer<input value={extra.rooms} onChange={(e) => updateExtra(row.property_id, "rooms", e.target.value)} placeholder="z.B. 4" /></label>
                      <label>Objektwert<input value={extra.marketValue} onChange={(e) => updateExtra(row.property_id, "marketValue", e.target.value)} placeholder="z.B. 350.000 €" /></label>
                    </div>
                    <SaveExtraBar propertyId={row.property_id} dirty={!!dirtyExtras[row.property_id]} status={extraStatus[row.property_id]} onSave={saveExtra} />
                  </section>

                  <section className="portfolio-edit-box">
                    <h3>Ausstattung</h3>
                    <label>Ausstattung / Merkmale<input value={extra.equipment} onChange={(e) => updateExtra(row.property_id, "equipment", e.target.value)} placeholder="Einbauküche, Keller, Balkon …" /></label>
                    <SaveExtraBar propertyId={row.property_id} dirty={!!dirtyExtras[row.property_id]} status={extraStatus[row.property_id]} onSave={saveExtra} />
                  </section>
                  ...
                  <section className="portfolio-edit-box expose-box">
                    ...
                    <div className="expose-actions">
                      ...
                      {uploadedExpose ? <a href={uploadedExpose.dataUrl} download={uploadedExpose.fileName}>Download</a> : null}
                    </div>
                    {uploadedExpose ? <small>Aktuell: {uploadedExpose.fileName}</small> : <small>Noch kein PDF hochgeladen. Du kannst trotzdem ein Exposé aus den aktuellen Daten erstellen.</small>}
                  </section>
                  ...
                  <section className="portfolio-edit-box wide">
                    ...
                    <div className="portfolio-input-grid four">
                      ...
                      <label>Telefon<input value={extra.phone} onChange={(e) => updateExtra(row.property_id, "phone", e.target.value)} placeholder="Telefon" /></label>
                      <label>E-Mail<input value={extra.email} onChange={(e) => updateExtra(row.property_id, "email", e.target.value)} placeholder="E-Mail" type="email" /></label>
                    </div>
                    <div className="portfolio-input-grid three rent">
                      <label>Kaltmiete<input value={extra.coldRent} onChange={(e) => updateExtra(row.property_id, "coldRent", e.target.value)} placeholder="z.B. 1.000,00 €" /></label>
                      <label>Betriebskosten / Nebenkosten<input value={extra.operatingCosts} onChange={(e) => updateExtra(row.property_id, "operatingCosts", e.target.value)} placeholder="z.B. 370,00 €" /></label>
                      <label>Gesamtmiete<input value={extra.totalRent} onChange={(e) => updateExtra(row.property_id, "totalRent", e.target.value)} placeholder="z.B. 1.370,00 €" /></label>
                    </div>
                    <SaveExtraBar propertyId={row.property_id} dirty={!!dirtyExtras[row.property_id]} status={extraStatus[row.property_id]} onSave={saveExtra} />
                  </section>
                </div>
      ...
      {selectedImage ? (
        <div className="portfolio-image-modal" role="dialog" aria-modal="true" onClick={() => setSelectedImage(null)}>
          <div className="portfolio-image-modal-card" onClick={(event) => event.stopPropagation()}>
            <button type="button" className="portfolio-image-modal-close" onClick={() => setSelectedImage(null)}>×</button>
            <img src={selectedImage.imageUrl} alt={selectedImage.title} />
            <div className="portfolio-image-modal-caption"><strong>{selectedImage.title}</strong><span>{selectedImage.subtitle}</span></div>
          </div>
        </div>
      ) : null}
          ...
          <div className="expose-modal-card" onClick={(event) => event.stopPropagation()}>
            <div className="expose-toolbar">
              ...
              {exposes[exposePreview.row.property_id] ? (
                <a href={exposes[exposePreview.row.property_id].dataUrl} download={exposes[exposePreview.row.property_id].fileName}>
            ...
            <div className="expose-print-sheet">
              <div className="expose-cover">
                {exposePreview.image ? <img src={exposePreview.image.imageUrl} alt={exposePreview.image.title} /> : null}
                <div><span>Immobilien-Exposé</span><h1>{exposePreview.row.property_name}</h1><p>{exposePreview.image?.subtitle ?? ""}</p></div>
              </div>
              ...
              <section><h2>Ausstattung</h2><p>{exposePreview.extra.equipment || "Keine Ausstattung hinterlegt."}</p></section>
              <section><h2>Finanzen</h2><p>Restschuld: <strong>{formatCurrency(exposePreview.row.last_balance)}</strong> · Tilgung gesamt: <strong>{formatCurrency(exposePreview.row.principal_total)}</strong> · Zinsen gesamt: <strong>{formatCurrency(exposePreview.row.interest_total)}</strong></p></section>
              <section><h2>Kontakt / Mieter</h2><p>{[exposePreview.extra.firstName, exposePreview.extra.lastName].filter(Boolean).join(" ") || "–"}<br />{exposePreview.extra.phone || ""}<br />{exposePreview.extra.email || ""}</p></section>
            </div>
          </div>
        </div>
      ) : null}
