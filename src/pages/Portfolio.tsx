...
export default function Portfolio() {
  ...
  useEffect(() => {
    ...
    async function loadPortfolio() {
      ...
      const normalized: PortfolioRowNormalized[] = ((data ?? []) as PortfolioRow[])
        .map((row) => ({
      ...
      <div style={styles.page}>
        <div style={styles.hero}>
          ...
          <p style={styles.heroText}>
            Übersicht über Darlehen, Tilgung, Zinsen, Objektzugriffe und hinterlegte Portfolio-Bilder.
          </p>
        </div>
  ...
  return (
    <div style={styles.page}>
      <div style={styles.hero}>
        <h1 style={styles.heroTitle}>Portfolio</h1>
        <p style={styles.heroText}>
          Übersicht über Darlehen, Tilgung, Zinsen und Objektzugriffe. Zusätzlich sind deine Objektbilder jetzt in sinnvoller Größe direkt unterhalb des Portfolios eingebunden.
        </p>
      </div>

      <section style={styles.gallerySection}>
        <div style={styles.galleryHeader}>
          <h2 style={styles.galleryTitle}>Objektbilder</h2>
          <p style={styles.galleryText}>
            Die Bilder aus deinem Anhang sind als große Galerie eingebunden, damit die Immobilien im Portfolio schneller visuell zugeordnet werden können.
          </p>
        </div>
        <div style={styles.galleryGrid}>
          {portfolioGalleryItems.map((item) => (
            <article key={item.id} style={styles.galleryCard}>
              <img src={item.imageUrl} alt={item.title} style={styles.galleryImage} />
              <div style={styles.galleryBody}>
                <h3 style={styles.galleryItemTitle}>{item.title}</h3>
                <div style={styles.galleryItemText}>{item.subtitle}</div>
              </div>
            </article>
          ))}
        </div>
              ...
              >
                <div style={styles.rowTop}>
                  <div style={styles.rowLeft}>
                    ...
                    <div style={styles.footerMeta}>Property-ID: {row.property_id}</div>
                  </div>

                  <div style={styles.actionArea}>
                    <button type="button" onClick={() => openSection(row, "address")} style={styles.button}>
                      Adresse
                    </button>

                    <button type="button" onClick={() => openSection(row, "details")} style={styles.button}>
                      Details
                    </button>

                    <button type="button" onClick={() => openSection(row, "finanzen")} style={styles.button}>
                      Finanzen
                    </button>

                    <button type="button" onClick={() => openSection(row, "energie")} style={styles.button}>
                      Energie
                    </button>

                    <button type="button" onClick={() => openSection(row, "vermietung")} style={styles.button}>
                      Vermietung
                    </button>
                  </div>
                </div>
