...
type ApartmentRow = { id: string; label: string; tenantName: string; area: number; allocationKey: number; persons: number; occupancyMonths: number; advancePayments: number; active: boolean };
...
export default function NebenkostenWohnungen() {
  ...
  const costBreakdown = useMemo(() => {
    ...
    return workspace.costs.map(row => {
      ...
      // CO₂-Automatik: Nur auf Heizkosten/KALO-Kosten anwenden.
      // Beispiel Colmarer 2025: 474,25 / 22.968,84 × 2.177,66 × 30% = ca. 13,49 € Vermieterabzug.
      ...
      const co2TenantShare = roundMoney(co2ShareApartment * (co2Stage.tenantPercent / 100));
      const co2LandlordShare = roundMoney(co2ShareApartment * (co2Stage.landlordPercent / 100));
  ...
  const totalCo2LandlordShare = roundMoney(costBreakdown.reduce((s, x) => s + x.co2LandlordShare, 0)); const totalCo2TenantShare = roundMoney(costBreakdown.reduce((s, x) => s + x.co2TenantShare, 0)); const totalHeatingBeforeCo2 = roundMoney(costBreakdown.filter(x => x.isHeating).reduce((s, x) => s + x.tenantShareBeforeCo2, 0));
  ...
  <Card title="Ergebnis" icon={<FileText className="h-5 w-5"/>} actions={<div className="flex gap-2"><button onClick={exportOnePager} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium"><FileText className="h-4 w-4"/> Export</button><button onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-medium"><Printer className="h-4 w-4"/> Drucken</button><button onClick={() => updateMeta("locked", true)} className="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-600 px-4 py-2 text-sm font-medium text-white"><CheckCircle2 className="h-4 w-4"/> Abschließen</button></div>}><div className="grid gap-4 md:grid-cols-4"><Stat title="Kalte Betriebskosten" value={formatCurrency(totalColdCosts)}/><Stat title="Heizkosten vor CO₂" value={formatCurrency(totalHeatingBeforeCo2)}/><Stat title="CO₂-Abzug Vermieter" value={`-${formatCurrency(totalCo2LandlordShare)}`} accent="success"/><Stat title="Heizkosten Mieter nach CO₂" value={formatCurrency(totalHeatingCosts)}/><Stat title="Gesamtkosten" value={formatCurrency(totalTenantCosts)}/><Stat title={tenantBalance >= 0 ? "Guthaben" : "Nachzahlung"} value={formatCurrency(Math.abs(tenantBalance))} accent={tenantBalance >= 0 ? "success" : "danger"}/></div><div className="mt-5 overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Kostenart</th><th className="px-4 py-3">Verteilung</th><th className="px-4 py-3">Schlüssel</th><th className="px-4 py-3 text-right">Mieter</th></tr></thead><tbody>{costBreakdown.map(x => <tr key={x.row.id} className="border-t"><td className="px-4 py-3 font-medium">{x.row.label}</td><td className="px-4 py-3">{allocationLabel(x.row.allocation)}</td><td className="px-4 py-3">{x.row.totalKey ? `${formatFlex(x.row.totalKey)} / ${formatFlex(x.row.apartmentKey)}` : "Direkt"}</td><td className="px-4 py-3 text-right font-semibold">{formatCurrency(x.tenantShare)}</td></tr>)}</tbody></table></div></Card></div></main>;
}
