...
type PortfolioFinanceSummary = {
  ...
  objekt_code?: string;
  jahr: number;
  einnahmen: number | string | null;
  ausgaben: number | string | null;
...
function getImage(name: string): PortfolioGalleryItem | undefined {
  ...
  return portfolioGalleryItems.find((item) =>
    item.matchTerms.some((term) => normalized.includes(normalizeText(term)))
  );
}
...
function InfoPill({
  ...
  return (
    <div className={`portfolio-data-pill ${tone}`}>
      <span>{label}</span>
      <b>{value}</b>
    </div>
  );
...
function SaveExtraBar({
  ...
  return (
    <div className="portfolio-save-row" style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 14 }}>
      ...
      {status ? (
        <small style={{ color: status.includes("Fehler") ? "#b91c1c" : "#475569", fontWeight: 700 }}>
          {status}
        </small>
      ) : null}
    </div>
...
export default function Portfolio() {
  ...
  const [financeSummary, setFinanceSummary] = useState<PortfolioFinanceSummary[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadFinanceSummary() {
      ...
      const { data, error } = await supabase
        .from("v_portfolio_finance_summary_jahr")
        .select("portfolio_property_id,user_id,name,objekt_code,jahr,einnahmen,ausgaben,mieteingaenge")
        .eq("jahr", year);

      if (cancelled) return;

      if (error) {
        console.error("Fehler beim Laden von v_portfolio_finance_summary_jahr:", error);
  ...
  const financeMap = useMemo(() => {
    const map: Record<string, PortfolioFinanceSummary> = {};

    financeSummary.forEach((item) => {
      if (item.portfolio_property_id) {
        map[item.portfolio_property_id] = item;
      }
    });
  ...
  const totals = useMemo(() => {
    return rows.reduce(
      (acc, row) => {
        ...
        const finance = row.portfolio_property_id ? financeMap[row.portfolio_property_id] : undefined;

        const income = toNumber(finance?.einnahmen);
        const expenses = toNumber(finance?.ausgaben);
  ...
  if (appData.loading) {
    return (
      <div className="portfolio-page">
        <div className="portfolio-state">Portfolio wird geladen…</div>
      </div>
    );
  }
  ...
  if (appData.error) {
    return (
      <div className="portfolio-page">
        <div className="portfolio-state error">Fehler beim Laden des Portfolios: {appData.error}</div>
      </div>
    );
  }

  return (
    <div className="portfolio-page portfolio-page-v11">
      <input
        ref={fileInputRef}
        type="file"
        accept="application/pdf"
        className="hidden-file-input"
        onChange={handleExposeUpload}
      />
      ...
      <header className="portfolio-hero compact polished">
        <div>
          ...
          <p>
            Live-Dashboard mit Buchungen als zentrale Datenquelle: Cashflow, Rendite, Mieten,
            Nebenkosten, Darlehen und Exposés pro Immobilie.
          </p>
          {financeError ? (
            <p style={{ color: "#b91c1c", fontWeight: 800 }}>
              Fehler beim Laden der Portfolio-Finanzwerte: {financeError}
            </p>
          ) : null}
        </div>
      </header>
      ...
      <section className="portfolio-kpis refined">
        ...
        <div>
          ...
          <b className={totals.cashflow >= 0 ? "positive" : "negative"}>
            {formatCurrency(totals.cashflow)}
          </b>
        </div>
        <div>
          <span>Brutto-Rendite</span>
          <b>{formatPercent(grossYield)}</b>
        </div>
        <div>
          <span>Netto-Rendite</span>
          <b>{formatPercent(netYield)}</b>
        </div>
        <div>
          <span>Restschuld gesamt</span>
          <b>{formatCurrency(totals.lastBalance)}</b>
        </div>
        <div>
          <span>Ø Rückzahlungsstand</span>
          <b>{formatPercent(averageRepaidPercent)}</b>
        </div>
      </section>
      ...
      <section className="portfolio-cashflow-panel">
        <div>
          ...
          <p>
            Einnahmen und Ausgaben kommen automatisch aus der SQL-View
            v_portfolio_finance_summary_jahr. Die Rendite nutzt den eingetragenen Objektwert;
            wenn kein Wert gepflegt ist, wird ersatzweise die Restschuld verwendet.
          </p>
        </div>
        ...
        <div className="cashflow-bars" aria-label="Cashflow Übersicht">
          <div>
            <span>Einnahmen</span>
            <i style={{ width: `${Math.min(100, safeRatio(totals.income, Math.max(totals.income, totals.expenses, 1)))}%` }} />
            <b>{formatCurrency(totals.income)}</b>
          </div>
          <div>
            <span>Ausgaben</span>
            <i style={{ width: `${Math.min(100, safeRatio(totals.expenses, Math.max(totals.income, totals.expenses, 1)))}%` }} />
            <b>{formatCurrency(totals.expenses)}</b>
          </div>
          <div>
            <span>Objektwerte</span>
            <i style={{ width: "100%" }} />
            <b>{formatCurrency(totals.portfolioValue)}</b>
          </div>
        </div>
      </section>
      ...
      <section className="portfolio-object-list refined-list">
        {rows.map((row) => {
          const extra = extraInfo[row.property_id] ?? emptyExtra;
          const image = getImage(row.property_name);
          ...
          const income = toNumber(finance?.einnahmen);
          const expenses = toNumber(finance?.ausgaben);
          const rentIncome = toNumber(finance?.mieteingaenge);

          const nebenkosten = appData
            .getNebenkostenExpenses(row.property_id, year)
          ...
          return (
            <article className="portfolio-object-card refined-card" key={row.property_id}>
              <div className="portfolio-object-main">
                <div className="portfolio-object-head refined-head">
                  <button
                    type="button"
                    className="portfolio-object-media small clickable"
                    onClick={() => image && setSelectedImage(image)}
                    aria-label={image ? `${image.title} vergrößern` : "Kein Objektbild vorhanden"}
                  >
                    {image ? (
                      <img src={image.imageUrl} alt={image.title} />
                    ) : (
                      <div className="portfolio-image-placeholder">Objektbild</div>
                    )}
                  </button>
                  ...
                  <div className="portfolio-actions top compact-actions">
                    ...
                    <button
                      type="button"
                      className="expose-main-button"
                      onClick={() => setExposePreview({ row, extra, image })}
                    >
                      Exposé
                    </button>
                  </div>
                ...
                <div className="portfolio-dashboard-grid refined-dashboard">
                  ...
                  <div className="portfolio-mini-card">
                    <span>Cashflow {year}</span>
                    <b className={net >= 0 ? "positive" : "negative"}>{formatCurrency(net)}</b>
                  </div>
                  <div className="portfolio-mini-card">
                    <span>Netto-Rendite</span>
                    <b>{formatPercent(objectYield)}</b>
                  </div>
                  <div className="portfolio-mini-card">
                    <span>Rückzahlung</span>
                    <b>{formatPercent(row.repaid_percent)}</b>
                ...
                <div className="portfolio-live-grid">
                  ...
                  <InfoPill
                    ...
                    tone={expenses > income ? "bad" : "neutral"}
                  />
                ...
                <div className="portfolio-info-grid refined-info">
                  <section className="portfolio-edit-box">
                    <h3>Eckdaten</h3>
                    <div className="portfolio-input-grid three">
                      <label>
                        Wohnfläche
                        <input
                          value={extra.livingArea}
                          onChange={(e) => updateExtra(row.property_id, "livingArea", e.target.value)}
                      ...
                      <label>
                        ...
                        <input
                          value={extra.rooms}
                          onChange={(e) => updateExtra(row.property_id, "rooms", e.target.value)}
                          placeholder="z.B. 4"
                        />
                      </label>
                      <label>
                        Objektwert
                        <input
                          value={extra.marketValue}
                          onChange={(e) => updateExtra(row.property_id, "marketValue", e.target.value)}
                    ...
                    <SaveExtraBar
                      ...
                      dirty={!!dirtyExtras[row.property_id]}
                      status={extraStatus[row.property_id]}
                      onSave={saveExtra}
                    />
                  </section>
                  ...
                  <section className="portfolio-edit-box">
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(e) => updateExtra(row.property_id, "equipment", e.target.value)}
                        placeholder="Einbauküche, Keller, Balkon …"
                      />
                    </label>
                    <SaveExtraBar
                      propertyId={row.property_id}
                  ...
                  <section className="portfolio-edit-box expose-box">
                    <h3>Exposé</h3>
                    <div className="expose-actions">
                      <button type="button" onClick={() => setExposePreview({ row, extra, image })}>Ansehen</button>
                      <button type="button" onClick={() => downloadGeneratedExpose(row, extra)}>PDF erstellen</button>
                      <button type="button" onClick={() => openUpload(row.property_id)}>PDF hochladen</button>
                      {uploadedExpose ? (
                        <a href={uploadedExpose.dataUrl} download={uploadedExpose.fileName}>Download</a>
                      ) : null}
                    </div>
                    {uploadedExpose ? (
                      <small>Aktuell: {uploadedExpose.fileName}</small>
                    ) : (
                      <small>Noch kein PDF hochgeladen. Du kannst trotzdem ein Exposé aus den aktuellen Daten erstellen.</small>
                    )}
                  </section>
                  ...
                  <section className="portfolio-edit-box wide">
                    ...
                    <div className="portfolio-input-grid four">
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) => updateExtra(row.property_id, "firstName", e.target.value)}
                          placeholder="Name"
                        />
                      </label>
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) => updateExtra(row.property_id, "lastName", e.target.value)}
                      ...
                      <label>
                        ...
                        <input
                          value={extra.phone}
                          onChange={(e) => updateExtra(row.property_id, "phone", e.target.value)}
                          placeholder="Telefon"
                        />
                      </label>
                      <label>
                        ...
                        <input
                          value={extra.email}
                          onChange={(e) => updateExtra(row.property_id, "email", e.target.value)}
                          placeholder="E-Mail"
                          type="email"
                        />
                      </label>
                    ...
                    <div className="portfolio-input-grid three rent">
                      <label>
                        Kaltmiete
                        <input
                          value={extra.coldRent}
                          onChange={(e) => updateExtra(row.property_id, "coldRent", e.target.value)}
                      ...
                      <label>
                        ...
                        <input
                          value={extra.operatingCosts}
                          onChange={(e) => updateExtra(row.property_id, "operatingCosts", e.target.value)}
                          placeholder="z.B. 370,00 €"
                        />
                      </label>
                      <label>
                        Gesamtmiete
                        <input
                          value={extra.totalRent}
                          onChange={(e) => updateExtra(row.property_id, "totalRent", e.target.value)}
                          placeholder="z.B. 1.370,00 €"
                        />
                      </label>
                    </div>
      ...
      {selectedImage ? (
        <div className="portfolio-image-modal" role="dialog" aria-modal="true" onClick={() => setSelectedImage(null)}>
          <div className="portfolio-image-modal-card" onClick={(event) => event.stopPropagation()}>
            <button type="button" className="portfolio-image-modal-close" onClick={() => setSelectedImage(null)}>
              ×
            </button>
            <img src={selectedImage.imageUrl} alt={selectedImage.title} />
            <div className="portfolio-image-modal-caption">
      ...
      {exposePreview ? (
        <div className="expose-modal" role="dialog" aria-modal="true" onClick={() => setExposePreview(null)}>
          <div className="expose-modal-card" onClick={(event) => event.stopPropagation()}>
            <div className="expose-toolbar">
              <button type="button" onClick={() => setExposePreview(null)}>Schließen</button>
              <button type="button" onClick={() => window.print()}>Als PDF speichern / drucken</button>
              {exposes[exposePreview.row.property_id] ? (
                <a
                  href={exposes[exposePreview.row.property_id].dataUrl}
                  download={exposes[exposePreview.row.property_id].fileName}
                >
                  Hochgeladenes PDF herunterladen
                </a>
              ) : null}
            </div>

            <div className="expose-print-sheet">
              <div className="expose-cover">
                {exposePreview.image ? (
                  <img src={exposePreview.image.imageUrl} alt={exposePreview.image.title} />
                ) : null}
                <div>
                  <span>Immobilien-Exposé</span>
                  <h1>{exposePreview.row.property_name}</h1>
                  <p>{exposePreview.image?.subtitle ?? ""}</p>
                </div>
              </div>

              <div className="expose-facts">
                <div><span>Wohnfläche</span><b>{exposePreview.extra.livingArea || "–"}</b></div>
                <div><span>Zimmer</span><b>{exposePreview.extra.rooms || "–"}</b></div>
                <div><span>Kaltmiete</span><b>{exposePreview.extra.coldRent || "–"}</b></div>
                <div><span>Nebenkosten</span><b>{exposePreview.extra.operatingCosts || "–"}</b></div>
                <div><span>Gesamtmiete</span><b>{exposePreview.extra.totalRent || "–"}</b></div>
                <div><span>Objektwert</span><b>{exposePreview.extra.marketValue || "–"}</b></div>
              </div>

              <section>
                <h2>Ausstattung</h2>
                <p>{exposePreview.extra.equipment || "Keine Ausstattung hinterlegt."}</p>
              </section>

              <section>
                <h2>Finanzen</h2>
                <p>
                  Restschuld: <strong>{formatCurrency(exposePreview.row.last_balance)}</strong> ·
                  Tilgung gesamt: <strong>{formatCurrency(exposePreview.row.principal_total)}</strong> ·
                  Zinsen gesamt: <strong>{formatCurrency(exposePreview.row.interest_total)}</strong>
                </p>
              </section>

              <section>
                <h2>Kontakt / Mieter</h2>
                <p>
                  {[exposePreview.extra.firstName, exposePreview.extra.lastName].filter(Boolean).join(" ") || "–"}
                  <br />
                  {exposePreview.extra.phone || ""}
                  <br />
                  {exposePreview.extra.email || ""}
                </p>
              </section>
            </div>
          </div>
